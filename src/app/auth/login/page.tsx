'use client';

import { useState, useEffect, Suspense } from 'react';
import { useSearchParams } from 'next/navigation';
import Link from 'next/link';
import Footer from '../../Footer';

function LoginContent() {
  const searchParams = useSearchParams();
  const inviteId = searchParams.get('invite');
  const [email, setEmail] = useState('');
  const [loading, setLoading] = useState(false);
  const [sent, setSent] = useState(false);
  const [error, setError] = useState('');

  useEffect(() => {
    const errorParam = searchParams.get('error');
    if (errorParam === 'google_failed') {
      setError('Google login was cancelled or failed.');
    } else if (errorParam === 'auth_error') {
      setError('A server error occurred during authentication. Please try again.');
    }
  }, [searchParams]);

  const handleMagicLink = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setError('');

    try {
      const res = await fetch('/api/auth/magic', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, inviteId }),
      });

      if (res.ok) {
        setSent(true);
      } else {
        const data = await res.json() as { error: string, details?: string };
        setError(data.details || data.error || 'Something went wrong');
      }
    } catch {
      setError('Failed to send magic link');
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="w-full max-w-sm space-y-8 flex flex-col justify-center">
      <div className="text-center">
        <Link href="/" className="text-2xl font-light tracking-tight text-[var(--primary)]">
          SayYes
        </Link>
        <h1 className="mt-6 text-xl font-medium text-[var(--primary)]">
          Welcome back
        </h1>
        <p className="mt-2 text-sm text-[var(--muted)]">
          Sign in to your weekly connection. SayYes is a private, intentional space designed to help couples maintain their bond through a simple weekly rhythm.
        </p>
      </div>

      <div className="space-y-4">
        <a
          href={`/api/auth/google${inviteId ? `?invite=${inviteId}` : ''}`}
          className="flex w-full items-center justify-center gap-3 bg-white border border-[var(--accent)] text-[var(--primary)] px-6 py-3 rounded-full font-medium hover:bg-stone-50 active:scale-[0.98] transition-all"
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path
              fill="currentColor"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="currentColor"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="currentColor"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"
            />
            <path
              fill="currentColor"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
          Continue with Google
        </a>

        <div className="relative flex items-center py-2">
          <div className="flex-grow border-t border-[var(--accent)]"></div>
          <span className="flex-shrink mx-4 text-xs text-[var(--muted)] uppercase tracking-widest">or email</span>
          <div className="flex-grow border-t border-[var(--accent)]"></div>
        </div>

        {sent ? (
          <div className="text-center p-6 bg-stone-50 rounded-2xl border border-[var(--accent)]">
            <p className="text-sm text-[var(--primary)] font-medium">Check your email</p>
            <p className="text-xs text-[var(--muted)] mt-1">We sent a magic link to {email}</p>
            <button 
              onClick={() => setSent(false)}
              className="mt-4 text-xs text-[var(--primary)] underline underline-offset-4"
            >
              Try a different email
            </button>
          </div>
        ) : (
          <form onSubmit={handleMagicLink} className="space-y-3">
            <input
              type="email"
              placeholder="email@example.com"
              required
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="w-full bg-white border border-[var(--accent)] text-[var(--primary)] px-5 py-3 rounded-full focus:outline-none focus:ring-1 focus:ring-[var(--primary)] transition-all"
            />
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[var(--primary)] text-white px-6 py-3 rounded-full font-medium hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-50"
            >
              {loading ? 'Sending...' : 'Send Magic Link'}
            </button>
            {error && <p className="text-center text-xs text-red-500 mt-2">{error}</p>}
          </form>
        )}
      </div>

      <div className="pt-12 border-t border-stone-100 space-y-6">
        <h2 className="text-sm font-medium text-[var(--primary)] uppercase tracking-widest">About SayYes</h2>
        <p className="text-xs text-[var(--muted)] leading-relaxed">
          SayYes is a calm, weekly relationship check-in designed to prevent drift and keep your connection intentional. 
          We believe that consistency is more powerful than intensity. By checking in once a week, you and your partner 
          can maintain alignment on emotional support, intimacy, logistics, and respect without the pressure of heavy clinical exercises.
        </p>
        <p className="text-xs text-[var(--muted)] leading-relaxed">
          Our private-first approach ensures that your notes remain yours alone, while your shared scores help you identify 
          where to focus your energy as a couple.
        </p>
        <div className="pt-2">
          <Link href="/how-it-works" className="text-[10px] text-[var(--primary)] uppercase tracking-widest font-medium hover:underline underline-offset-4">
            Learn more about how it works →
          </Link>
        </div>
      </div>
    </div>
  );
}

export default function LoginPage() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center py-20 bg-[var(--background)]">Loading...</div>}>
      <LoginContent />
    </Suspense>
  );
}
